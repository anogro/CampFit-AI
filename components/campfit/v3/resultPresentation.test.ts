...
describe("CampFit v3 result presentation", () => {
  ...
  it("distinguishes verified, development example, and unavailable catalog sources", () => {
    const verified = programCatalogPresentation("supabase")
    const example = programCatalogPresentation("static_fallback")
    ...
    expect(example.showProgramCards).toBe(true)
    expect(example.sectionTitle).toBe("개발용 예시 후보")
    expect(example.notice).toContain("실제 프로그램 DB 후보가 아니며")
    const demo = programCatalogPresentation("demo")
    expect(demo.sectionTitle).toBe("추천 프로그램 예시")
    expect(demo.sectionSubtitle).toContain("시연용 후보")
    expect(demo.notice).toContain("실제 운영 일정·가격·모집 여부는 제공기관 확인이 필요합니다")
    expect(demo.notice).not.toContain("가짜")
