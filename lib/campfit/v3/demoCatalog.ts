...
export function loadDemoCatalog(referenceYear = new Date().getUTCFullYear()): V3Catalog {
  return {
    ...
    cities: demoCityDefinitions.map((city): V3CatalogCity => ({
      ...
      profile: {
        safetyLevel: city.profile.safetyLevel,
        medicalLevel: city.profile.medicalLevel,
        internationality: city.profile.internationality,
        activityStrength: city.profile.natureStrength === "high" ? "high" : "medium",
        natureStrength: city.profile.natureStrength,
        strengths: city.profile.strengths,
        evidence: city.parentStayEvidence,
      },
    })),
    source: "demo",
...
function mapDemoProgram(definition: (typeof demoProgramDefinitions)[number], year: number): V3CatalogProgram {
  ...
  return {
    ...
    englishRequirement: {
      ...
      beginnerParticipation: definition.beginnerClass,
