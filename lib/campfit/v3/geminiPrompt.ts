...
export function buildConversationPrompt(input: AnalyzeConversationInput): string {
  ...
  return [
    ...
    "표현이 스키마의 라벨과 달라도 의미를 이해해 정규화하세요. 예를 들어 영어유치원·영어 수업 경험·간단한 대화·수업 참여 가능처럼 아이가 영어를 접했고 수업을 따라갈 수 있다는 설명은 childEnglishLevel로 변환해야 하며, 원문에 beginner/basic/intermediate 같은 단어가 없다는 이유로 unresolved에 남기거나 같은 질문을 반복하지 마세요.",
    "아이 영어 수준은 능력 중심으로 판단하세요: 영어 노출·단어 수준은 beginner/basic, 간단한 문장·일상 대화·영어 수업 참여가 가능하면 intermediate, 자유로운 대화와 수업 이해가 분명하면 advanced입니다. 표현이 다소 모호해도 추천에 충분한 가장 보수적인 등급을 confidence 0.85 이상으로 기록하세요.",
