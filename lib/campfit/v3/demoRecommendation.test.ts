...
describe("CampFit v3 Demo Catalog recommendation coverage", () => {
  it("returns three real demo cities and programs for the representative family scenario", () => {
    ...
    expect(result.programCandidates.length).toBeGreaterThanOrEqual(3)
    expect(result.programCandidates.length).toBeLessThanOrEqual(9)
    ...
    expect(new Set(result.programCandidates.map((program) => program.cityName)).size).toBe(3)
    expect(result.programCandidates.every((program) => result.destinationRecommendations.some((city) => city.cityName === program.cityName))).toBe(true)
    const programCountsByCity = result.programCandidates.reduce<Record<string, number>>((counts, program) => ({ ...counts, [program.cityName]: (counts[program.cityName] ?? 0) + 1 }), {})
    expect(Object.values(programCountsByCity).every((count) => count >= 1 && count <= 3)).toBe(true)
